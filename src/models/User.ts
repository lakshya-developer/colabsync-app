...
export const EmployeeCounterModel =
  mongoose.models.EmployeeCounter ||
  mongoose.model("EmployeeCounter", employeeCounterSchema);
...
UserSchema.pre<User>("save", async function () {
  ...
  let counter = await EmployeeCounterModel.findByIdAndUpdate(
    counterId,
    { $inc: { number: 1 }, $setOnInsert: { block: "A" } },
    { new: true, upsert: true },
  );

  if (counter.number > 9999) {
    counter = await EmployeeCounterModel.findByIdAndUpdate(
      counterId,
      {
        number: 1,
        block: nextBlock(counter.block),
      },
      { new: true },
    );
  }
...
UserSchema.index({ "meta.employeeCode": 1 }, { unique: true, sparse: true });
